...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (String[] coord : COORDINATES) {
            Thread thread = new Thread(() -> buscarDadosClimaticos(coord, mapaDadosClimaticos));
            threads.add(thread);
            thread.start();
        }
    ...
    private static void buscarDadosClimaticos(String[] coord, Map<String, dadosClimaticos> mapaDadosClimaticos) {
        ...
        try {
            ...
            if (response.statusCode() == 200) {
            	dadosClimaticos weatherData = processResponse(response.body());
            	mapaDadosClimaticos.put(latitude + "," + longitude, weatherData);
            } else {
                System.out.println("Failed to fetch data for coordinates: " + latitude + ", " + longitude);
            }
    ...
    private static void displayDadosClimaticos(Map<String, dadosClimaticos> weatherDataMap) {
        for (Map.Entry<String, dadosClimaticos> entry : weatherDataMap.entrySet()) {
